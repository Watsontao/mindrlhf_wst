...
class PartialSingleTurnAgentLoop(AgentLoopBase):
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        # P-DSR Dynamic Repack Trigger
        # If generation hit the dynamic cap (which is less than the absolute response_length limit), 
        # we consider it "interrupted" so it triggers the repack to the HEAVY worker.
        dynamic_cap = sampling_params.get("max_tokens", self.response_length)
        if len(response_ids) >= dynamic_cap and dynamic_cap < self.response_length:
            is_cancel = True
