...
class DetachNcclSync(AsyncActorRolloutRefWorker):
    ...
    def sync_rollout_weights(self, sync_group_name="actor_rollout"):
        ...
        weight_checksum = 0.0  # For verifying transfer integrity
        for key, shape, dtype in self._weights_info:
            ...
            # [HCCL FIX] Ensure the async HCCL broadcast completes before reusing the buffer
            get_torch_device().synchronize()

            if self._is_rollout:
                inference_model.load_weights([(key, tensor)])
                weight_checksum += tensor.float().sum().item()

        # [HCCL FIX] Global barrier to ensure all ranks finished all broadcasts
        from ray.util.collective import collective as collective_barrier
        collective_barrier.barrier(group_name=sync_group_name)
        get_torch_device().synchronize()

        if self._is_rollout:
            print(f"[sync_rollout_weights] Rollout weight checksum: {weight_checksum:.4f} "
                  f"(total {len(self._weights_info)} tensors loaded)")
