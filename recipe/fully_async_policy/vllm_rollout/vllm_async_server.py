...
# P-DSR: Diagnostic import wrapper — we need to see the REAL error
# when this import fails, not just "cannot import name 'FullyAsyncvLLMReplica'".
# The most common failure mode is that verl/workers/rollout/vllm_rollout/__init__.py
# triggers import of vLLMAsyncRollout which cascades into torch.distributed/vllm
# initialization failure inside the Ray Actor process.
...
try:
    ...
    print(f"[P-DSR FATAL] Failed to import from verl core vllm_async_server.")
    print(f"[P-DSR FATAL] Error type: {type(_import_err).__name__}")
    print(f"[P-DSR FATAL] Error: {_import_err}")
    _tb.print_exc()
    # Try to give more context about what's in sys.modules
    import sys as _sys
    _pkg = "verl.workers.rollout.vllm_rollout"
    if _pkg in _sys.modules:
        _mod = _sys.modules[_pkg]
        print(f"[P-DSR DIAG] Package '{_pkg}' IS in sys.modules: {_mod}")
        print(f"[P-DSR DIAG] Package dir: {dir(_mod)}")
    else:
        print(f"[P-DSR DIAG] Package '{_pkg}' is NOT in sys.modules")
    _tgt = "verl.workers.rollout.vllm_rollout.vllm_async_server"
    if _tgt in _sys.modules:
        _tmod = _sys.modules[_tgt]
        print(f"[P-DSR DIAG] Module '{_tgt}' IS in sys.modules: {_tmod}")
        print(f"[P-DSR DIAG] Module dir: {dir(_tmod)}")
    else:
        print(f"[P-DSR DIAG] Module '{_tgt}' is NOT in sys.modules")
    raise
