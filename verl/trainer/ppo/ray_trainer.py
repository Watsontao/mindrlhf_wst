            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    try:
                        ...
                        if hasattr(_response_mask, 'cpu'):
                            _response_mask = _response_mask.cpu()
                        _lengths = _response_mask.sum(dim=-1).tolist()

                        _responses_tensor = batch.batch['responses']
                        if hasattr(_responses_tensor, 'cpu'):
                            _responses_tensor = _responses_tensor.cpu()
                        _decoded_texts = self.tokenizer.batch_decode(_responses_tensor, skip_special_tokens=True)

                        _model_path = self.config.actor_rollout_ref.model.path
                        _model_match = _re.search(r'(\d+(?:\.\d+)?b)', _model_path.lower())
                        _model_suffix = _model_match.group(1) if _model_match else "model"

                        _output_dir = os.path.join(os.path.expanduser("~"), "work", "verl_070", "Rollout_response", "sync")
                        os.makedirs(_output_dir, exist_ok=True)

                        _n = self.config.actor_rollout_ref.rollout.n
                        _timestamp = _time.time()
                        
                        with open(os.path.join(_output_dir, f"rollout_length_stats_sync_{_model_suffix}.jsonl"), "a", encoding="utf-8") as _f_len, \
                             open(os.path.join(_output_dir, f"rollout_responses_sync_{_model_suffix}.jsonl"), "a", encoding="utf-8") as _f_txt:
                             
                            # Group responses by prompt (each prompt has _n responses)
                            for i in range(0, len(_lengths), _n):
                                _prompt_lengths = _lengths[i:i+_n]
                                _prompt_texts = _decoded_texts[i:i+_n]
                                
                                _log_entry = {
                                    "timestamp": _timestamp,
                                    "global_steps": self.global_steps,
                                    "n_responses": len(_prompt_lengths),
                                    "lengths": _prompt_lengths,
                                }
                                _f_len.write(json.dumps(_log_entry, ensure_ascii=False) + "\n")

                                _log_entry_text = {
                                    "global_steps": self.global_steps,
                                    "responses": _prompt_texts,
                                }
                                _f_txt.write(json.dumps(_log_entry_text, ensure_ascii=False) + "\n")
                    except Exception as _e:
                        print(f"[RayPPOTrainer] Failed to log rollout responses: {_e}")
